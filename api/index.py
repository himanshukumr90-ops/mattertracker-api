...
def get_case():
    ...
    try:
        ...
        # ── All petitioners (for full detail) ──────────
        petitioners = []
        ...
        is_disposed = bool(data.get("disposal_date") or
                          (isinstance(status_raw, dict) and
                           status_raw.get("status_type") == "M"))

        # ── Dates ──────────────────────────────────────
        # Next hearing date — for pending cases
        next_date = clean_date(
            data.get("next_date") or
            data.get("cause_list_date") or
